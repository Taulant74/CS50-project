...
export default function VehiclesPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    axios
      .get<Vehicle[]>('https://localhost:7072/api/vehicles')
      .then((res) => {
        if (!mounted) return
  ...
  return (
    <div className="page-container vehicles-page">
      <header className="mb-6">
        ...
        <p className="text-sm text-gray-500">Browse available cars from VirtuRide.</p>
      </header>
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="w-1/2 px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
      ...
      <section>
        {filtered.length === 0 ? (
          <div className="text-center text-gray-500">No vehicles match your search/filters.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((v) => (
              <div
                ...
                className="bg-white rounded-lg shadow hover:shadow-lg transition transform hover:-translate-y-1 cursor-pointer overflow-hidden"
              >
                <div className="w-full h-44 bg-gray-100">
                  <img
                    src={v.images && v.images.length ? v.images[0] : `https://picsum.photos/seed/vehicle${v.id}/800/480`}
                    alt={`${v.brand} ${v.model}`}
                ...
                <div className="p-4 flex flex-col gap-2">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="text-sm text-gray-500">{v.brand}</div>
                      <div className="font-semibold text-lg">{v.model}</div>
                    </div>
                    <div className="text-indigo-600 font-bold">{fmtPrice(v.price)}</div>
                  </div>
