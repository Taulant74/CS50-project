...
const InquiryPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Replace with real POST /api/inquiries
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 text-gray-50 flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-xl bg-gray-950/90 border border-gray-800 rounded-3xl shadow-2xl p-8">
        ...
        <p className="text-xs text-gray-400 mb-5">
          Send us a direct message about availability, history, or anything
          else you want to know.
        </p>

        <form onSubmit={handleSubmit} className="space-y-4 text-sm">
          <div className="rounded-xl bg-gray-900 border border-gray-800 px-3 py-2 text-xs text-gray-300 mb-1">
            Vehicle ID: <span className="text-gray-100 font-semibold">#{vehicleId}</span>
          </div>
          ...
          <div>
            ...
            <textarea
              className="w-full rounded-lg bg-gray-900 border border-gray-800 px-3 py-2 text-xs text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[100px]"
              placeholder="Ask about availability, price, history, trade-in options, etc."
              value={message}
          ...
          {success && (
            <div className="rounded-lg bg-emerald-500/10 border border-emerald-500/60 text-emerald-200 text-xs px-3 py-2">
              Your inquiry has been sent. We will reply to your email as soon as
              possible.
            </div>
          )}
